...
def newPull():
	...
	#             request_duration = time.time() - start
		...
		json = pd.Series({"PERCENT_COMPLETE" : 100, "ERROR" : 0}).to_json(orient="index")
		...
		json = pd.Series({"PERCENT_COMPLETE" : 100, "ERROR" : 1}).to_json(orient="index")
