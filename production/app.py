...
# search error
# save files to csv
# hard limit on scraping vids 500+
# make feedback button asynchronous, add name and email fields (add these as new columns in bugs table)
# Email server for api key requests and feedback?
# Create API - API endpoint and parameters in API Page, have API KEY Request feature
### Make api endpoint to get all artist_codes in db?
...
# print(app.config['SQLALCHEMY_DATABASE_URI'])s

# FIRST REQUEST FUNCTION
def newPull(name_key):
	try:
		...
		connection = create_engine(os.environ['DATABASE_URL'])

		# Creating database if not exists
		# connection.execute("CREATE DATABASE youtubedata")
		# connection.execute("USE youtubedata")
		...
		for playlist_link in playlist_urls:    
			...
			# global cancel
			# shutdown() 
			print(f"job meta in urls extraction before refresh : {job.meta}")
			# # print(cancel)
			job.refresh()
			print(f"job meta in urls extraction after refresh: {job.meta}")
			if job.meta["KILL"] == 1:
				kill_binary = 1
				# cancel = 0
				# percent_complete = 100
		...
		#     print(first_video_within_playlist)
			...
			print(f"total_videos_all is : {total_videos_all}")

			if total_videos_all > 500:
				kill_binary=1
				job.refresh()
				job.meta["TOO_MANY"] = 1
				job.save_meta()
				raise ValueError("Too many videos (500+)...killing job")
			...
			if total_videos_in_playlist == 1:
				...
				one_vid_json = pd.Series({"ERROR" : 1,
										  "REASON": "channel only has 1 video"}).to_json(orient="index")
	...
	#             request_duration = time.time() - start
		...
		for i in range(len(urls_all)):
			print(f"job meta in video extraction before refresh: {job.meta}")
			job.refresh()
			print(f"job meta in video extraction after refresh: {job.meta}")
			# shutdown() 
			if job.meta["KILL"] == 1:
				#cancel = 0
				kill_binary =1
				...
				# job.meta['PERCENT_COMPLETE'] = percent_complete
				# job.save_meta()
				# job.refresh()
				# print(job.meta)
				raise ValueError('A cancel request was submitted, cancelling process.')
		...
		ARTIST_CODE VARCHAR(255) NOT NULL UNIQUE\
		)")
		...
		json = pd.Series({"PERCENT_COMPLETE" : 101, "ERROR" : 0, "KILL":kill_binary}).to_json(orient="index")
		connection.dispose()
		...
		print(f"error: {e}")
		if e.args[0] == "A cancel request was submitted, cancelling process.":
			job.refresh()
			job.meta['PERCENT_COMPLETE'] = 101
			job.meta['ERROR'] = 0
			#job.meta["KILL"] = kill_binary
		...
		print(f"something went wrong on pull route! : {e}")
		site = "pull"
		# connection.execute("\
		...
		json = pd.Series({"PERCENT_COMPLETE" : 101, "ERROR" : 1,"KILL":kill_binary}).to_json(orient="index")
		connection.dispose()
		return json
...
def newScrape(name_key):
	try:
		...
		# Creating database if not exists
		# connection.execute("CREATE DATABASE IF NOT EXISTS youtubedata")
		# connection.execute("USE youtubedata")
		...
		#     print(first_video_within_playlist)
			...
			#     print("Getting urls...")
				...
				for i in range(total_videos_in_playlist):  
					# shutdown() 
					# global cancel
					print(f"job meta in url extraction before refresh: {job.meta}")
					job.refresh()
					print(f"job meta in url extraction after refresh: {job.meta}")
					if job.meta["KILL"] == 1:
						kill_binary =1
						#cancel = 0
						print("Cancelling this scrape...")
	...
	#             request_duration = time.time() - start
		...
		print("moving artist up in queue...")
		...
		VALUES ('{scrape_date}', '{artist}','{artist_code}')")
		...
		json = pd.Series({"PERCENT_COMPLETE" : 101, "ERROR" : 0}).to_json(orient="index")
		job.refresh()
		job.meta['PERCENT_COMPLETE'] = 101
		job.meta['ERROR'] = 0
		#job.meta["KILL"] = kill_binary
...
def percent():
	...
	# kill_key = request.args.get("kill")
	job = Job.fetch(job_key, connection=conn)
	job.refresh()
	print(f"JOB META from loading route: {job.meta}")

	if job.is_finished:
		print(f"results json, job done: {job.result}")
		return jsonify(job.meta)

	# if kill_key == 1:
	# 	print(f"killing {job_key}")
	# 	# print(f"all workers {worker.all()}")
	# 	#cancel_job(job)
	...
	# 	killed_json = pd.Series({"JOB_KEY" : job_key,
	# 							 "KILLED_STATUS" : 1,
	# 							 "PERCENT_COMPLETE" : 101,
	# 							 "ERROR" : 0}).to_json(orient="index")

	# 	return killed_json

	percent_series = pd.Series({"PERCENT_COMPLETE":job.meta['PERCENT_COMPLETE'],
...
def sendCard():
	deck = int(request.args["deck"])
	infinite_counter = deck*12
...
def update_route():
	...
	result = q.enqueue(newScrape, name_key)
	job_key = str(result.get_id())
	job = Job.fetch(job_key, connection=conn)
	job.meta['PERCENT_COMPLETE'] = 100
	job.meta["ERROR"] = 0
	job.meta["KILL"] = 0
	job.meta["TOO_MANY"] = 0
	job.save_meta()
	#job.refresh()
	print(f"inital job meta: {job.meta}")
	print(f"jobs in queue: {len(q)}")
	# ENQUEUE REDIS HERE
	# Pass Job ID in Function so meta can be updated
	json_return = pd.Series({"JOB_STARTED" : 1,
							 "JOB_KEY" : job_key}).to_json(orient="index")

	return json_return

# New Scrape Request ddd
@app.route("/pull")
def new_request():
	name_key = request.args["name"]
	print(f"name key: {name_key}")
	print("enqueueing job")
	result = q.enqueue(newPull, name_key)
	job_key = str(result.get_id())
...
cancel=cancel,videos_to_get=videos_to_get):
	try:
		...
		connection.dispose()
		print(f"something went wrong in the home route: {e}")
		return "needs more records in db"
...
cancel=cancel):
	try:
		...
		if content_creator != "None":
			...
			print(f"db scrape date: {scrape_date}")
			days_since_scraped = str(round((datetime.now() - scrape_date).days))
			print(f"days since scraped: {days_since_scraped}")
			...
			days_since_scraped=days_since_scraped)
		...
		if input_analytics == "base":
			...
			print(f"db scrape date: {scrape_date}")
			days_since_scraped = str(round((datetime.now() - scrape_date).days))
			print(f"days since scrape: {days_since_scraped}")
		...
		try:
			...
			# Checking Database to See if Data was Previously Scraped
				...
				search_json = pd.Series({"YOUTUBE_CODE" : youtube_code_new_scrape,
							"ARTIST_NAME":artist_name_new_scrape,
							"SUBSCRIBERS":subscribers_int_new_scrape,
							"TOTAL_VIEWS":total_views_int_new_scrape,
							"JOINED":joined_convert_new_scrape,
							"IMAGE_URL":artist_image_new_scrape,
							"TOTAL_VIDEOS":videos_to_get,
							"IN_DB" : 0}).to_json(orient="index")
		...
		if len(df_cache) != 0:
			...
			search_json = pd.Series({"YOUTUBE_CODE" : artist_db_name,
		...
		search_json = pd.Series({"PERCENT_COMPLETE" : 100,
								 "ERROR" : 1}).to_json(orient="index")
...
@app.route("/error")
def error_route():
	real_reason = request.args.get("overflow")
	eight_ball=["It is certain",
				"It is decidedly so",
				"Without a doubt",
				"Yes - definitely",
	...
	if real_reason == "1":
		return render_template("error.html", reason="We currently do not support scraping YouTube channels with 500+ videos.")
	
	else:
		return render_template("error.html", reason=reason)


@app.route("/killJob", methods=["GET"])
def kill_job():
	if request.method == "GET":
		...
		kill_binary =1
		job.save_meta()
		print(f"job meta kill route before refresh: {job.meta}")
		job.refresh()
		print(f"job meta: {job.meta}")
		print(f"job meta kill route after refresh: {job.meta}")
		killed_json = pd.Series({"JOB_KEY" : job_key,
								 "KILLED_STATUS" : 1}).to_json(orient="index")
...
def delete_artist():
	...
	if request.method == "GET":
		...
		print(f"deleting {name} from database tables...")

		connection.execute(f"DELETE FROM requests WHERE artist_code = '{name}'")
		print(f"deleted artist from requests table...")

		connection.execute(f"DELETE FROM artists WHERE artist_code = '{name}'")
		print(f"deleted artist from artists table...")

		connection.execute(f"DELETE FROM searches WHERE artist_code = '{name}'")
		print(f"deleted artist from searches table...")

		connection.execute(f"DROP TABLE {name}")
		print(f"dropped artist table entirely...")
...
def api_endpoint():
	...
	if request.method == "GET":
		...
		channel_code = request.args.get("channel_code").replace("-","_replaced_").strip()
		print(f"channel_code : {channel_code}")
		...
		keys = pd.read_sql("SELECT api_key FROM approved_keys",connection)["api_key"].tolist()

		all_requests = pd.read_sql("SELECT requests FROM approved_keys",connection)["requests"].tolist()

		if api_key in keys:

			api_index = keys.index(api_key)
			total_requests = all_requests[api_index]

			try:
				requests = pd.read_sql(f"SELECT requests FROM api_keys where api_key = '{api_key}'", connection).iloc[0][0]
				print("this api key has been used before...")
				api_key_exists = 1

			except Exception as e:
				requests = 0
				print(f"this api key has NOT been used before...error: {e}")
				connection.execute(f"INSERT INTO api_keys(api_key,requests) VALUES('{api_key}', {requests})")
				api_key_exists = 0
			
			try:
				if requests <= total_requests:
					api_json = pd.read_sql(f"SELECT * FROM {channel_code}", connection).drop(["id","search_name","published_str","comments"],axis="columns")
					api_json["artist_code"] = api_json["artist_code"].str.replace("_replaced_","-")
					api_json = api_json.to_json(orient="records")
					requests += 1

					connection.execute(f"UPDATE api_keys SET requests = {requests} WHERE api_key = '{api_key}'")
					connection.execute(f"INSERT INTO api_requests(api_key,channel_code) VALUES('{api_key}','{channel_code}')")

				else:
					api_json = pd.Series({"ERROR" : "api key has reached maximum uses"}).to_json(orient="index")

			except Exception as e:

				print(f"Error in getting data for API call...{e}")
				api_json = pd.Series({"ERROR" : "does not exist in database"}).to_json(orient="index")

		else:

			print(f"Invalid api key...")
			api_json = pd.Series({"ERROR" : "invalid api key"}).to_json(orient="index")
